...
- 登录和 OAuth 回调验证 `Origin`/`Referer` 与 `SIDERVIA_PUBLIC_URL`；生产 Cookie 必须 Secure。
...
- Attempt 绑定 admin session、Provider、Account 草稿和 egress fingerprint。
...
每日 UTC 00:10 聚合前一日并幂等 upsert；每月聚合由日聚合生成。删除 365 天明细前先确认对应日/月聚合存在并记录清理审计事件。
