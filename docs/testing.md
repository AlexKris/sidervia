...
- Native response path 保留未知 Provider JSON 字段/事件且剥离敏感字段；官方 Provider 的未知 request field 返回 `unknown_request_field`，自定义兼容 Upstream 只转发管理员显式允许的字段路径。
...
| --- | --- | --- | --- | --- | --- |
| 文本非流式/流式 | planned | planned | planned | planned | planned |
...
| API Key | planned | planned | planned | planned | planned |
| OAuth/Device/WIF | planned | planned | planned | planned | planned |
...
fake proxy 为每个连接记录身份，断言 authorization、token、validation、quota 和 inference 都使用 Account 解析出的同一 Egress Profile。修改代理后，新快照使用新连接池，旧活跃请求安全结束。
...
v0.1 已实现的自动化用例和仍为 planned 的范围记录在[实现状态](implementation-status.md)。在 Provider、调度、资源、媒体或用量代码出现前，对应章节是必须保留的验收契约，不得标记为已通过。
